...
<head>
    ...
    <title>iFamily Card - Kartu Keluarga</title>
        ...
        <main>
          <div class="container">
            ...
            <table width=100% border=1>
              ...
              <tr class="tbody">
                <form action="/kk/create" method="get">
                  <button type="submit" class="btn btn-success">Tambah KK</button>
                </form>
                @if ($errors->any())
                ...
                <tr>
                    ...
                    <td>
                        <form action="{{ url('/daftar/'.$item['id'].'/keluarga') }}">
                            <button class="btn btn-primary">Anggota</button>
                        </form>
                        <form action="{{ url('/kk/'.$item['id'].'/edit') }}">
                            <button class="btn btn-warning">Edit</button>
                        </form>
                        <form action="{{ url('/kk/'.$item['id']) }}" method="POST" class="d-inline">
                            @method('delete')
                            @csrf
